...
def apply_denoising_NLM(piece):
    return cv2.fastNlMeansDenoisingColored(piece, None, 2, 1, 7, 21)
...
def apply_sharpening_laplacian(piece):
    ...
    return cv2.filter2D(piece, -1, kernel)
    ...
    for subfolder in SUBFOLDERS:
        ...
        for file in files:
            ...
            for idx, piece in enumerate(raw_pieces):
                piece_name = f"{base_name}_piece_{idx}.png"
                
                save_piece(piece, "1_sliced", subfolder, piece_name)

                denoised_piece = apply_denoising_NLM(piece)
                save_piece(denoised_piece, "2_denoised", subfolder, piece_name)

                enhanced_piece = apply_enhancement(piece)
                save_piece(enhanced_piece, "3_enhanced", subfolder, piece_name)
